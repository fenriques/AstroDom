    ...
    def plot(self, imageListModel):
        ...
        for row in range(imageListModel.rowCount()):
            ...
            date_time_obj = datetime.datetime.strptime(
                datetimestr[row], '%Y-%m-%dT%H:%M:%S.%f')
            ...
            # create data sets
            g1.append(
                {'pos': (alt[row], az[row]), 'brush': colors[row]})
            g2.append(
                {'pos': (az[row], alt[row]), 'brush': colors[row]})
            g3.append(
                {'pos': (timestampObj[row], alt[row]), 'brush': colors[row]})
            g4.append(
                {'pos': (alt[row], fwhm[row]), 'brush': colors[row]})
            g5.append(
                {'pos': (az[row], fwhm[row]), 'brush': colors[row]})
            g6.append(
                {'pos': (timestampObj[row], fwhm[row]), 'brush': colors[row]})
            g7.append(
                {'pos': (alt[row], eccentricity[row]), 'brush': colors[row]})
            g8.append(
                {'pos': (az[row], eccentricity[row]), 'brush': colors[row]})
            g9.append(
                {'pos': (timestampObj[row], eccentricity[row]), 'brush': colors[row]})
            g10.append(
                {'pos': (alt[row], noise[row]), 'brush': colors[row]})
            g11.append(
                {'pos': (az[row], noise[row]), 'brush': colors[row]})
            g12.append(
                {'pos': (timestampObj[row], noise[row]), 'brush': colors[row]})
            g13.append(
                {'pos': (alt[row], snrweight[row]), 'brush': colors[row]})
            g14.append(
                {'pos': (az[row], snrweight[row]), 'brush': colors[row]})
            g15.append(
        ...
        # Graph1: Alt-Az
        self.ui.graphWidget1.setLabel(
            'left', 'Az (deg)', color='white', size=30)
        self.ui.graphWidget1.setLabel(
            'bottom', 'Alt (deg)', color='white', size=30)
        self.ui.graphWidget1.showGrid(x=True, y=True)

        scatter1 = pg.ScatterPlotItem(
            brush=pg.mkBrush(width=5, color='w'), symbol='o')
        ...
        # Graph2: AZ-Alt
        self.ui.graphWidget2.setLabel(
            'left', 'Alt (deg)', color='white', size=30)
        self.ui.graphWidget2.setLabel(
            'bottom', 'Az (deg)', color='white', size=30)
        ...
        scatter3.setData(g3)
        self.lrDate = pg.LinearRegionItem(
            [min(timestampObj), max(timestampObj)])
        self.lrDate.setZValue(-10)
        self.ui.graphWidget3.addItem(self.lrDate)

        # Graph4: Alt-FWHM
        self.ui.graphWidget4.setLabel(
            'bottom', 'Alt (deg)', color='white', size=30)
        self.ui.graphWidget4.setLabel(
            'left', 'FWHM', color='white', size=30)
        ...
        # Graph5: Az-FWHM
        self.ui.graphWidget5.setLabel(
            'bottom', 'Az (deg)', color='white', size=30)
        self.ui.graphWidget5.setLabel(
            'left', 'FWHM', color='white', size=30)
        self.ui.graphWidget5.showGrid(x=True, y=True)

        scatter5 = pg.ScatterPlotItem(
            brush=pg.mkBrush(width=5, color='w'), symbol='o')
        self.ui.graphWidget5.addItem(scatter5)
        scatter5.setData(g5)

        # Graph6: Time-FWHM
        self.ui.graphWidget6.setLabel(
            'left', 'FWHM', color='white', size=30)
